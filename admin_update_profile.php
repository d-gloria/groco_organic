...
// admin_header.php garanton që je admin i vlefshëm.
// Por na duhet profili për formën.
$admin_id = (int)($_SESSION['admin_id'] ?? 0);

// Merr profilin e adminit
...
if(isset($_POST['update_profile'])){
   ...
   if(isset($_FILES['image']) && !empty($_FILES['image']['name'])){
      ...
      if(!in_array($ext, $allowed, true)){
         ...
         $ok = $update_image->execute([$new_image, $admin_id]);

         if($ok){
            move_uploaded_file($image_tmp, $image_folder);

            // mos fshi default
            if(!empty($old_image) && $old_image !== 'default.png' && file_exists('uploaded_img/'.$old_image)){
   ...
   if($wants_password_change){
      ...
      if($old_password_input === '' || $new_password === '' || $confirm_password === ''){
         ...
         if(!password_verify($old_password_input, (string)$fetch_profile['password'])){
            ...
            $update_pass_query->execute([$new_hash, $admin_id]);
